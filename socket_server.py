...
def handle_client(conn: socket.socket, addr: Tuple[str, int], task_queue: multiprocessing.Queue, max_days: int = 3):
    # Crear un AppointmentManager para lecturas (usa la ventana configurada)
    am = AppointmentManager(max_days=max_days)
    with conn:
        ...
        while True:
            ...
            if parts[0].upper() == 'LIST':
                ...
                slots = am.list_available(date)
                if not slots:
                    conn.sendall(b"No hay turnos disponibles.\n")
                else:
                    for s in slots:
                        conn.sendall((pretty_slot(s) + "\n").encode('utf-8'))
                ...
                try:
                    ...
                    task_queue.put(task)
                    conn.sendall(b"Reserva encolada. El worker la procesará pronto.\n")
                ...
                try:
                    ...
                    conn.sendall(b"Cancelación encolada por id.\n")
                except Exception:
                ...
                if not name:
                    ...
                    conn.sendall(b"Cancelación encolada por nombre.\n")

            elif parts[0].upper() in ('QUIT', 'EXIT'):
                conn.sendall(b"Adiós\n")
                break
